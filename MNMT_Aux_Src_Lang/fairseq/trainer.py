...
class Trainer(object):
    ...
    def model(self):
        if self._wrapped_model is None:
            if self.data_parallel_world_size > 1 and not self.args.use_bmuf and not self.args.use_hit:
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        if self._stats_group is None and self.args.use_hit:
            if not self.args.fp16:
                hit_optimizer = self._optimizer._optimizer
            elif self.args.memory_efficient_fp16:
                hit_optimizer = self._optimizer.wrapped_optimizer._optimizer
            else:
                hit_optimizer = self._optimizer.fp32_optimizer._optimizer
            self._stats_group = hit_optimizer.get_sync_group(0)
        ...
        try:
            ...
            if self.args.use_hit:
                self.optimizer.multiply_grads(8 / float(sample_size))
            ...
            if not self.args.use_bmuf and not self.args.use_hit:
